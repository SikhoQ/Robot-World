...
public class TextWorld implements IWorld {
    ...
    public Robot launchRobot(String make, String name) {
        if (isValidRobotName(name)) {
            ...
            System.out.println("Invalid robot name. Please enter a valid name.");
            return null;
        }
    }

    private boolean isValidRobotName(String name) {
        return !name.isEmpty() && !isRobotNameExists(name);
    }

    private boolean isRobotNameExists(String name) {
        for (Robot robot : robots) {
